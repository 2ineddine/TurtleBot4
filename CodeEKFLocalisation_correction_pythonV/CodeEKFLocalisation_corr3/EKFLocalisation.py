...
def ekf_localisation():
    ...
    # Estimated covariance matrices - TEST CONFIGURATIONS FOR QUESTION 3
    
    # Configuration 1: Perfect match (baseline)
    QEst = QTrue.copy()
    PYEst = PYTrue.copy()
    
    # Configuration 2: Very low perception noise (overconfident in sensors)
    # QEst = QTrue.copy()
    # PYEst = 0.01 * PYTrue.copy()
    # -> Filter becomes overconfident in measurements
    # -> Can lead to divergence or poor tracking
    
    # Configuration 3: Large distance noise, correct bearing noise
    # QEst = QTrue.copy()
    # PYEst = np.array([[100, 0], [0, 1]]) * PYTrue
    # -> Filter underestimates distance measurement quality
    # -> More uncertainty in position estimates
    # -> Better robust to distance measurement outliers
    
    # Configuration 4: Underestimated process noise
    # QEst = 0.01 * QTrue.copy()
    # PYEst = PYTrue.copy()
    # -> Filter becomes overconfident in motion model
