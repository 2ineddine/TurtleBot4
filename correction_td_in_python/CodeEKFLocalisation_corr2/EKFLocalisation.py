...
def get_observation(k):
    """Get observation with noise - includes sensor failure simulation"""
    global Map, xTrue, PYTrue, nSteps
    
    # Fake sensor failure - same logic as MATLAB
    if abs(k - nSteps/2) < 0.1*nSteps:
        z = None  # Empty observation
        iFeature = -1
    else:
...
def do_graphs(InnovStore, PStore, SStore, XStore, XErrStore):
    ...
    plt.figure(1)
    plt.savefig('EKFLocation.eps', format='eps')
    ...
    plt.xlabel('time')
    plt.savefig('EKFLocationInnov.eps', format='eps')
    ...
    plt.ylabel('θ (deg)')
    plt.xlabel('time')
    plt.savefig('EKFLocationErr.eps', format='eps')
...
def ekf_localisation():
    ...
    # Clear all and close figures (equivalent to MATLAB's close all; clear all)
    plt.close('all')
    ...
    # Main EKF loop
    for k in range(2, nSteps + 1):  # MATLAB uses 1-based indexing
        # Simulate world
        simulate_world(k)
        ...
        if z is not None:  # if(~isempty(z)) in MATLAB
            # Predict observation
        ...
        # Store results (convert to 0-based indexing for Python)
        k_idx = k - 1
        InnovStore[:, k_idx] = Innov
        PStore[:, k_idx] = np.sqrt(np.diag(PEst))
        if not np.isnan(S).any():
            SStore[:, k_idx] = np.sqrt(np.diag(S))
        XStore[:, k_idx] = xEst
        XErrStore[:, k_idx] = xTrue - xEst
