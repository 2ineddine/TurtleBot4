...
% Modeled errors used in the Kalman filter process
%QEst = eye(3,3)*QTrue;
%PYEst = 0.01*eye(2,2)*PYTrue;
% -> trop grande confiance dans le capteur
% -> bruits importants dans les estimés (particulièrement la direction)
% Modeled errors used in the Kalman filter process
QEst = eye(3,3)*QTrue;
PYEst = [100 0 ; 0 1]*PYTrue;
% -> trop faible confiance dans la mesure de la distance
% -> bruit supposé fort sur la distance mesurée
% -> + forte erreur sur la position en (x,y) 
% => importance de bien estimer les bruits 
%    + de dessiner les bornes sigma (=indication
%      d'une mauvaise/bonne estimation des bruits/si le EKF est bien "réglé")
